...
def ncl_expected_score(team_rating: float, opponent_rating: float) -> float:
    return 1 / (1 + 10 ** ((opponent_rating - team_rating) / 400))


def ncl_series_score(team_wins: int, opponent_wins: int) -> float:
    return (team_wins + 0.5) / (team_wins + opponent_wins + 1)


def ncl_new_rating(team_rating: int, opponent_rating: int, team_wins: int, opponent_wins: int) -> int:
    expected = ncl_expected_score(team_rating, opponent_rating)
    score = ncl_series_score(team_wins, opponent_wins)
    return round(team_rating + 64 * (score - expected))


def ncl_rating_update(
    team1_rating: int,
    team2_rating: int,
    team1_wins: int,
    team2_wins: int,
    ...
    return (
        ncl_new_rating(team1_rating, team2_rating, team1_wins, team2_wins),
        ncl_new_rating(team2_rating, team1_rating, team2_wins, team1_wins),
    )
...
def generate_ncl_schedule(team_ids: list[int], start_date: date) -> list[ScheduledPair]:
